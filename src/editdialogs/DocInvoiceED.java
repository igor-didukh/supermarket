...
public class DocInvoiceED extends EditDialogDoc implements ActionListener {
	...
	protected Entity entityFromFields() {
		...
		return
			new DocInvoice(
				...
				client.getId(),
				client.getKlient().getImie(),
				client.getKlient().getNazwisko(), 
				store.getId(), 
				store.getMagazynier().pobierzImie(),
				store.getMagazynier().pobierzNazwisko(),
				0
			);
	}
